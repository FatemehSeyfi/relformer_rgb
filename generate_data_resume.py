...
def save_input(path, idx, patch, patch_seg, patch_coord, patch_edge):
    ...
    raw_path = f"{path}raw/sample_{str(idx).zfill(6)}_data.png"
    seg_path = f"{path}seg/sample_{str(idx).zfill(6)}_seg.png"
    vtp_path = f"{path}vtp/sample_{str(idx).zfill(6)}_graph.vtp"

    # اگر نمونه قبلاً ذخیره شده، رد شو
